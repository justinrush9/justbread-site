...
DOUGH_STEPS = [
    ...
    ("Rest",180),("Preshape",240),("Shape",330),("-> Fridge",390),
]
DOUGH_MIN = 390
...
# Dough must be in the fridge by this time of day (24h clock). The whole dough
# timeline is back-scheduled from here:  mix = FRIDGE_BY - DOUGH_MIN (7.5 hrs).
# At 2:00 PM this puts the mix at 6:30 AM. Change these two numbers to shift it.
FRIDGE_BY_H, FRIDGE_BY_M = 14, 0   # 2:00 PM
...
# ── PDF builder (inlined from former pdf_builder.py) ───────────────────────────
# reportlab canvas layout for a printable, checkbox-style worksheet.
F_PDF  = "Helvetica"
FB_PDF = "Helvetica-Bold"
FS_TITLE = 13; FS_GEN = 5.5; FS_TOTAL_LBL = 5.0; FS_TOTAL_VAL = 7.5
FS_DAY = 9.0; FS_ACT = 5.5; FS_DET = 5.5; FS_BODY = 6.5; FS_COLHDR = 5.0
LH = 8.0; ACT_H = 10.0; DET_H = 8.0; DAY_H = 13.0; CB_SZ = 5.0; CB_X = 1.5; CB_TX = 8.5
G_DAY = 0.83; G_S = 0.78; G_SB = 0.90; G_D = 0.72; G_DB = 0.87; G_B = 0.65; G_BB = 0.82


def build_pdf(totals, day_acts, days, dough_steps, fmt_fn, grams_fn,
              supply_fn, bag_g, gallon_g, now_str, landscape=True):
    buf = BytesIO()
    PW, PH = (792.0, 612.0) if landscape else (612.0, 792.0)
    NCOLS  = 3 if landscape else 2
    MARGIN = 18.0; GAP = 10.0; HDR_H = 44.0
    col_w  = (PW - 2*MARGIN - (NCOLS-1)*GAP) / NCOLS

    def col_x(ci): return MARGIN + ci*(col_w + GAP)
    content_top = PH - MARGIN - HDR_H - 4
    content_bot = MARGIN
    c = rlc.Canvas(buf, pagesize=(PW, PH))

    def bg(x, y_top, w, h, g):
        c.setFillColorRGB(g, g, g)
        c.rect(x, y_top - h, w, h, fill=1, stroke=0)

    def checkbox(x, y_top, sz=CB_SZ, lw=0.8):
        c.setStrokeColorRGB(0.25, 0.25, 0.25)
        c.setFillColorRGB(1, 1, 1)
        c.setLineWidth(lw)
        c.rect(x, y_top - sz - 1, sz, sz, fill=1, stroke=1)

    def row(x, y_top, w, lbl, val="", bold=False, cb=True, lbl_grey=0.2, val_grey=0.0):
        if cb: checkbox(x + CB_X, y_top)
        tx = x + (CB_TX if cb else 3)
        c.setFont(FB_PDF if bold else F_PDF, FS_BODY)
        c.setFillColorRGB(lbl_grey, lbl_grey, lbl_grey)
        c.drawString(tx, y_top - LH + 1.5, lbl)
        if val:
            c.setFont(FB_PDF if bold else F_PDF, FS_BODY)
            c.setFillColorRGB(val_grey, val_grey, val_grey)
            c.drawRightString(x + w - 1, y_top - LH + 1.5, val)
        return LH

    def act_header(x, y_top, w, label, detail, bg_g):
        bg(x, y_top, w, ACT_H, bg_g)
        c.setFont(FB_PDF, FS_ACT)
        c.setFillColorRGB(0.05, 0.05, 0.05)
        c.drawString(x + CB_TX, y_top - ACT_H + 2, label.upper())
        y2 = y_top - ACT_H
        c.setFont(F_PDF, FS_DET)
        c.setFillColorRGB(0.3, 0.3, 0.3)
        c.drawString(x + CB_TX, y2 - DET_H + 2, detail)
        return ACT_H + DET_H

    def day_header(x, y_top, w, name):
        bg(x, y_top, w, DAY_H, G_DAY)
        checkbox(x + CB_X, y_top - (DAY_H - CB_SZ)/2 + CB_SZ, sz=CB_SZ+1, lw=1.0)
        c.setFont(FB_PDF, FS_DAY)
        c.setFillColorRGB(0, 0, 0)
        c.drawString(x + CB_TX + 3, y_top - DAY_H + 3.5, name)
        return DAY_H
    ...
    def measure_day(acts):
        ...
        for atype, data in acts:
            h += ACT_H + DET_H
            if atype == 'starter':   h += 3 * LH
            elif atype == 'dough':   h += (4 + len(dough_steps)) * LH
            elif atype == 'bake':
                nb = data.get('batches', 1)
                h += LH + 8 + nb * LH + 2 * LH
            h += 4
        h += 4
        return h

    active = [(di, days[di], day_acts[di]) for di in range(7) if day_acts[di]]
    slots  = []
    pg, ci, cy = 0, 0, content_top
    for di, dname, acts in active:
        h = measure_day(acts)
        if h > cy - content_bot:
            ci += 1
            if ci >= NCOLS:
                ci = 0; pg += 1
            cy = content_top
        slots.append((pg, ci, cy, dname, acts))
        cy -= h + 5
    num_pages = (max(s[0] for s in slots) + 1) if slots else 1

    def draw_hdr():
        c.setFont(FB_PDF, FS_TITLE)
        c.setFillColorRGB(0, 0, 0)
        c.drawString(MARGIN, PH - MARGIN - 11, "JUSTBREAD")
        c.setFont(F_PDF, FS_GEN)
        c.setFillColorRGB(0.45, 0.45, 0.45)
        c.drawString(MARGIN, PH - MARGIN - 19, f"Weekly Production Schedule   {now_str}")
        bar_items = [
            ("LOAVES",            str(totals["loaves"])),
            ("STARTING STARTER",  grams_fn(totals["fridge_starter"])),
            ("FLOUR",             totals["flour"]),
            ("WATER",             totals["water"]),
            ("SALT",              totals["salt"]),
        ]
        bw = (PW - 2*MARGIN) / len(bar_items)
        by = PH - MARGIN - 24; bh = 17.0
        for i, (lbl, val) in enumerate(bar_items):
            ...
            c.setFont(FB_PDF, FS_TOTAL_LBL)
            c.setFillColorRGB(0.35, 0.35, 0.35)
            c.drawString(bx + 3, by - 7, lbl)
            c.setFont(FB_PDF, FS_TOTAL_VAL)
            c.setFillColorRGB(0, 0, 0)
            c.drawString(bx + 3, by - 15.5, val)

    def draw_day(x, y_top, w, dname, acts):
        y = y_top
        y -= day_header(x, y, w, dname)
        y -= 2
        bold_steps = {"Mix","Add Salt","Laminate 1","Laminate 2",
                      "Rest","Preshape","Shape","-> Fridge"}
        for atype, data in acts:
            loaves = int(data["tag"].split()[0])
            bake_d = data["tag"].split("->")[-1].strip()
            if atype == "starter":
                y -= act_header(x, y, w, "Starter Feeding",
                                f"{loaves} loaves for {bake_d}", G_S)
                rows_h = 3 * LH
                bg(x, y, w, rows_h, G_SB)
                for lbl, val, bld in [
                    ("AM Feed - pull from fridge",   grams_fn(data["pull"]),   False),
                    ("PM Feed - flour & water each", grams_fn(data["after1"]), False),
                    ("After PM Feed - use for dough",grams_fn(data["total"]),  True),
                ]:
                    row(x, y, w, lbl, val, bold=bld)
                    y -= LH
            elif atype == "dough":
                y -= act_header(x, y, w, "Make Dough",
                                f"{loaves} loaves for {bake_d}   "
                                f"start {fmt_fn(data['mix'])}", G_D)
                rows_h = (4 + len(dough_steps)) * LH
                bg(x, y, w, rows_h, G_DB)
                for lbl, val in [
                    ("Flour",   supply_fn(data["flour"], bag_g, "bag")),
                    ("Water",   supply_fn(data["water"], gallon_g, "gallon")),
                    ("Salt",    grams_fn(data["salt"])),
                    ("Starter", grams_fn(data["starter"])),
                ]:
                    row(x, y, w, lbl, val, cb=False, lbl_grey=0.35)
                    y -= LH
                for nm, t in data["steps"]:
                    row(x, y, w, nm, fmt_fn(t), bold=(nm in bold_steps))
                    y -= LH
            elif atype == "bake":
                nb = data["batches"]
                y -= act_header(x, y, w, "Bake",
                                f"{loaves} loaves   {nb} batch{'es' if nb!=1 else ''}"
                                f"   done by {fmt_fn(data['done_by'])}", G_B)
                for nm, t in data["events"]:
                    if "Oven ON" in nm:
                        bg(x, y, w, LH, G_BB)
                        row(x, y, w, "Oven ON  (preheat)", fmt_fn(t),
                            cb=False, lbl_grey=0.4, val_grey=0.2)
                        y -= LH
                        break
                bg(x, y, w, 8, G_BB)
                c.setFont(FB_PDF, FS_COLHDR)
                c.setFillColorRGB(0.35, 0.35, 0.35)
                c.drawString(x + CB_TX,      y - 6, "#")
                c.drawString(x + CB_TX + 16, y - 6, "LOAVES")
                c.drawRightString(x + w - 1,  y - 6, "IN")
                c.setStrokeColorRGB(0.35, 0.35, 0.35)
                c.setLineWidth(0.5)
                c.line(x, y - 7.5, x + w, y - 7.5)
                y -= 8
                for nm, t in data["events"]:
                    if "IN" not in nm: continue
                    m   = _re.search(r'Batch (\d+)', nm)
                    rng = _re.search(r'\(([^)]+)\)', nm)
                    if not m: continue
                    bg(x, y, w, LH, G_BB)
                    checkbox(x + CB_X, y)
                    c.setFont(F_PDF, FS_BODY)
                    c.setFillColorRGB(0.2, 0.2, 0.2)
                    c.drawString(x + CB_TX,      y - LH + 1.5, f"#{m.group(1)}")
                    c.drawString(x + CB_TX + 16, y - LH + 1.5, rng.group(1) if rng else "")
                    c.setFillColorRGB(0, 0, 0)
                    c.drawRightString(x + w - 1, y - LH + 1.5, fmt_fn(t))
                    y -= LH
                for nm, t in data["events"]:
                    if nm not in ("Packaging", "DONE"): continue
                    bg(x, y, w, LH, G_BB)
                    row(x, y, w, nm, fmt_fn(t), bold=(nm == "DONE"), cb=False,
                        lbl_grey=(0 if nm=="DONE" else 0.3))
                    y -= LH
            y -= 4
    ...
    for p in range(num_pages):
        if p > 0: c.showPage()
        draw_hdr()
        for pg, ci, y_start, dname, acts in slots:
            if pg != p: continue
            draw_day(col_x(ci), y_start, col_w, dname, acts)
    c.save()
    buf.seek(0)
    return buf.read()
...
class App(tk.Tk):
    ...
    def _calc(self):
        ...
        for bake_idx, bake_day, loaves, done_dt in active:
            ...
            # Dough timeline is back-scheduled from a fixed "in fridge by" target
            # (FRIDGE_BY), NOT from the next day's bake time. This keeps the mix at
            # a sane hour instead of the small hours of the morning.
            fridge_dt = r5(done_dt.replace(hour=FRIDGE_BY_H, minute=FRIDGE_BY_M,
